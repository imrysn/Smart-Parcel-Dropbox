...
io.on('connection', (socket) => {
  ...
  socket.on('verifyScan', async ({ trackingId, mode }) => {
    ...
    try {
      // Check if this is an Owner QR Access Badge (generated by mobile app)
      const isOwnerBadge = trackingId && (trackingId.startsWith('SPD-QR-') || trackingId.startsWith('SPD-OWNER-'));
      if (isOwnerBadge) {
        console.log(`🔑 OWNER BADGE AUTHORIZED AT PHYSICAL SCANNER: ${trackingId}`);
        await ScanLog.create({
          scannedId: trackingId,
          accessGranted: true,
          mode: mode,
          status: 'authorized',
          reason: 'owner_badge_scan'
        });
  ...
  socket.on('registerDevice', async ({ token, userId, deviceName }) => {
    ...
    const expiresAt = entry?.expiresAtMs || entry?.expiresAt;
    if (!entry || (expiresAt && Date.now() > expiresAt)) {
  ...
  socket.on('pushHardwareConfig', async ({ ssid, password }) => {
    ...
    if (socket.userId) {
      await Dropbox.findOneAndUpdate(
        { $or: [{ userIds: socket.userId }, { primaryUserId: socket.userId }, { userId: socket.userId }] },
        { $set: { wifiSSID: ssid } }
      ).catch(() => {});
    }
  });
  ...
  socket.on('hardwareConfigApplied', async ({ deviceId }) => {
    ...
    try {
      ...
      if (dropbox) {
        const targetUsers = (dropbox.userIds && dropbox.userIds.length > 0)
          ? dropbox.userIds
          : [dropbox.primaryUserId || dropbox.userId].filter(Boolean);

        targetUsers.forEach(uid => {
          io.to(uid).emit('hardwareConfigApplied', { deviceId });
        });
        io.emit('hardwareConfigApplied', { deviceId });
      } else {
        io.emit('hardwareConfigApplied', { deviceId });
      }
    } catch (err) {
