/*
 * Smart Parcel Dropbox - PRODUCTION IoT VERSION
 * ============================================
...
// User ID - Get this from your Flutter app after login
// For testing, hardcode your MongoDB user ID here
const char* USER_ID = "REPLACE_WITH_YOUR_USER_ID_FROM_MONGODB";
...
void onSocketEvent(socketIOmessageType_t type, uint8_t * payload, size_t length) {
  switch(type) {
    ...
    case sIOtype_CONNECT:
      ...
      // Join user's room
      String joinMessage = "[\"join\",\"" + String(USER_ID) + "\"]";
      webSocket.emit("join", joinMessage.c_str());
      
      Serial.println("Joined room: " + String(USER_ID));
    ...
    case sIOtype_EVENT:
      {
        ...
        if (!error) {
          ...
          // Handle doorStateUpdate event from Flutter app
