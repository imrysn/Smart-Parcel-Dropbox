...
      'accessGranted': accessGranted,
      'timestamp': Timestamp.fromDate(timestamp),
      // Only include optional fields if they are not null (from 'main')
      if (trackingId != null) 'trackingId': trackingId,
      if (userId != null) 'userId': userId,
      if (reason != null) 'reason': reason,
    };
...
  /// Get formatted date and time string (from 'Brant', with small modification to reuse the main version's logic for the 7+ day case)
  String getFormattedDateTime() {
    final date = timestamp;
    final now = DateTime.now();
    final difference = now.difference(date);
...
    if (difference.inDays == 0) {
      if (difference.inMinutes < 1) { // Adjusted from Brant's 'Just now' logic
        return 'Just now';
      } else if (difference.inHours == 0) {
        return '${difference.inMinutes} minute${difference.inMinutes == 1 ? '' : 's'} ago';
      }
...
      return 'Yesterday at ${_formatTime(date)}';
    } else if (difference.inDays < 7) {
      return '${difference.inDays} day${difference.inDays > 1 ? 's' : ''} ago'; // Using logic similar to 'main' here
    } else {
      return '${_formatDate(date)} at ${_formatTime(date)}';
    }
  }
...
  /// Get full formatted date and time (from 'Brant')
  String getFullFormattedDateTime() {
    final date = timestamp;
    final day = date.day.toString().padLeft(2, '0');
    final month = date.month.toString().padLeft(2, '0');
...
  /// Get status color indicator (from 'main')
  String getStatusColor() {
    return accessGranted ? 'green' : 'red';
  }
}
>>>>>>> 188bd5ffd30b49c825dcafe3212dd5d0358ce1ac
